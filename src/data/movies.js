export const movies = [
  {
    id: 1,
    title: 'Втеча з Шоушенка',
    description: 'Двоє ув’язнених знаходять спільну мову та надію на свободу через роки дружби.',
    genre: 'Драма',
    date: '14.05.2025',
    time: '19:00',
    poster: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQWpWQ_Pn-Zxk_QFHlQWAtwvSGdlxvZ39Ihew&s',
    trailer: 'https://www.youtube.com/embed/NmzuHjWmXOc',
    hall: 1
  },
  {
    id: 2,
    title: 'Темний лицар',
    description: 'Коли з’являється Джокер, місто Готем занурюється у хаос, і Бетмен має зупинити загрозу.',
    genre: 'Бойовик, Кримінал, Драма',
    date: '10.06.2025',
    time: '21:30',
    poster: 'https://upload.wikimedia.org/wikipedia/uk/8/8a/Dark_Knight.jpg',
    trailer: 'https://www.youtube.com/embed/EXeTwQWrcwY',
    hall: 1
  },
  {
    id: 3,
    title: 'Початок',
    description: 'Злодій використовує технологію спільного сну, щоб проникати в підсвідомість своїх жертв.',
    genre: 'Бойовик, Фантастика',
    date: '10.07.2025',
    time: '18:30',
    poster: 'https://upload.wikimedia.org/wikipedia/uk/e/e1/%D0%9F%D0%BE%D1%87%D0%B0%D1%82%D0%BE%D0%BA_%D1%84%D1%96%D0%BB%D1%8C%D0%BC%2C_2010.jpg',
    trailer: 'https://www.youtube.com/embed/YoHD9XEInc0',
    hall: 2
  },
  {
    id: 4,
    title: 'Форрест Гамп',
    description: 'Проста людина з великим серцем переживає дивовижні пригоди, змінюючи історію США.',
    genre: 'Драма, Романтика',
    date: '22.05.2025',
    time: '17:00',
    poster: 'https://uaserial.com/images/serials/64/640b4fbde39dc024980740.webp',
    trailer: 'https://www.youtube.com/embed/bLvqoHBptjg',
    hall: 1
  },
  {
    id: 5,
    title: 'Інтерстеллар',
    description: 'Група дослідників подорожує через космос, щоб врятувати людство.',
    genre: 'Фантастика, Драма, Пригоди',
    date: '01.08.2025',
    time: '20:15',
    poster: 'https://upload.wikimedia.org/wikipedia/uk/2/29/Interstellar_film_poster2.jpg',
    trailer: 'https://www.youtube.com/embed/2LqzF5WauAw',
    hall: 3
  },
  {
    id: 6,
    title: '1+1: Недоторканні',
    description: 'Історія про дружбу між багатим інвалідом і доглядальником з нетиповим підходом до життя.',
    genre: 'Комедія, Драма',
    date: '01.07.2025',
    time: '19:45',
    poster: 'https://uakino.me/uploads/posts/2017-11/1511342116_r6j98qa7clsvpvymc3x6zpci07.jpg',
    trailer: 'https://www.youtube.com/embed/34WIbmXkewU',
    hall: 4
  },
  {
    id: 7,
    title: 'Матриця',
    description: 'Хакер дізнається, що реальність — це симуляція, і приєднується до боротьби за свободу.',
    genre: 'Фантастика, Бойовик',
    date: '10.09.2025',
    time: '22:00',
    poster: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcS530tChTweGZ5S4L171ePfubDbNVL4zu13zg&s',
    trailer: 'https://www.youtube.com/embed/vKQi3bBA1y8',
    hall: 3
  },
  {
    id: 8,
    title: 'Паразити',
    description: 'Бідна родина поступово проникає в життя заможної сім’ї, що веде до трагічних наслідків.',
    genre: 'Драма, Трилер',
    date: '05.10.2025',
    time: '18:00',
    poster: 'https://upload.wikimedia.org/wikipedia/uk/8/84/Parasite_2019_poster.jpg',
    trailer: 'https://www.youtube.com/embed/isOGD_7hNIY',
    hall: 2
  },
  {
    id: 9,
    title: 'Джокер',
    description: 'Ставши відомим злочинцем, комік-невдаха збожеволів і повертається до хаосу.',
    genre: 'Драма, Трилер, Кримінал',
    date: '01.11.2025',
    time: '20:00',
    poster: 'https://uakino.me/uploads/posts/2019-10/157147479435_joker.jpg',
    trailer: 'https://www.youtube.com/embed/zAGVQLHvwOY',
    hall: 4
  },
  {
    id: 10,
    title: 'Гаррі Поттер і філософський камінь',
    description: 'Хлопчик-чарівник вступає до школи магії.',
    genre: 'Пригоди, Фентезі',
    date: '15.12.2025',
    time: '16:00',
    poster: 'https://upload.wikimedia.org/wikipedia/uk/c/c5/%D0%93%D0%9F%D0%A4%D0%9A%D0%9F%D0%BE%D1%81%D1%82%D0%B5%D1%80.jpg',
    trailer: 'https://www.youtube.com/embed/l91Km49W9qI',
    hall: 4
  },
  {
    id: 11,
    title: 'Месники: Завершення',
    description: 'Остання битва супергероїв за долю всього всесвіту.',
    genre: 'Бойовик, Фантастика, Пригоди',
    date: '15.08.2025',
    time: '21:00',
    poster: 'https://upload.wikimedia.org/wikipedia/uk/d/d4/%D0%9C%D0%B5%D1%81%D0%BD%D0%B8%D0%BA%D0%B8_%D0%97%D0%B0%D0%B2%D0%B5%D1%80%D1%88%D0%B5%D0%BD%D0%BD%D1%8F_%D0%94%D1%80%D1%83%D0%B3%D0%B8%D0%B9_%D0%9F%D0%BE%D1%81%D1%82%D0%B5%D1%80.png',
    trailer: 'https://www.youtube.com/embed/TcMBFSGVi1c',
    hall: 3
  },
  {
    id: 12,
    title: 'Король Лев',
    description: 'Сімба, молодий лев, повинен подолати вигнання, щоб повернути законне місце короля.',
    genre: 'Мультфільм, Пригоди',
    date: '10.09.2025',
    time: '15:30',
    poster: 'https://upload.wikimedia.org/wikipedia/ru/6/62/Lion_king_ver1.jpg',
    trailer: 'https://www.youtube.com/embed/7TavVZMewpY',
    hall: 4
  },
  {
    id: 13,
    title: 'Соціальна мережа',
    description: 'Історія створення соцмережі Facebook та боротьба за контроль над компанією.',
    genre: 'Біографія, Драма',
    date: '12.10.2025',
    time: '19:30',
    poster: 'https://upload.wikimedia.org/wikipedia/uk/d/d1/%D0%A1%D0%BE%D1%86%D1%96%D0%B0%D0%BB%D1%8C%D0%BD%D0%B0_%D0%BC%D0%B5%D1%80%D0%B5%D0%B6%D0%B0_%D1%84%D1%96%D0%BB%D1%8C%D0%BC_2010.jpg',
    trailer: 'https://www.youtube.com/embed/lB95KLmpLR4',
    hall: 2
  },
  {
    id: 14,
    title: 'Титанік',
    description: 'Роуз згадує час на «Титаніку», де вона зустрічає Джека під час першого плавання корабля.',
    genre: 'Драма, Романтика',
    date: '01.12.2025',
    time: '20:45',
    poster: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTwD7b_p_4s1E0NNXAwsTQ4guf3BmpjMRi7gQ&s',
    trailer: 'https://www.youtube.com/embed/kVrqfYjkTdQ',
    hall: 2
  }
];
